...
from .exceptions import QuarantineBypassError
...
class KnowledgeGraph:
    ...
    def add_node(self, node: Node):
        self.nodes[str(node.entity_id)] = node
    ...
    def quarantine_cascade(self, node_id: uuid.UUID):
        node = self.require_node(node_id)
        node.quarantine_flag = True
        # Recursively quarantine targets of this node
        for rel in self.relations:
            if str(rel.source_node_id) == str(node_id):
                try:
                    self.quarantine_cascade(rel.target_node_id)
                except ValueError:
                    pass
